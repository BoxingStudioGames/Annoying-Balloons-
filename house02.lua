...
-- background 
local background01 = display.newImageRect("bg/drop-01.png", 320, 570)
 background01.x = display.contentWidth/2
 background01.y = display.contentHeight/2 
 screenGroup:insert(background01)
 ---showing background 
 local background = display.newImageRect("bg/level13.png", 320, 570)
...
badStar010 = balloonClass02.new02()
screenGroup:insert(badStar010)

badStar011 = balloonClass02.new02()
screenGroup:insert(badStar011)

badStar012 = balloonClass02.new02()
screenGroup:insert(badStar012)
...
transH07 = transition.to( sub, {time = 8000, delay = 2000, x = math.random(100,500), y = display.contentHeight-700, onComplete = nil} )
...
transH08 = transition.to( subp, {time = 5000, delay = 2000, x = math.random(100,500), y = display.contentHeight-700, onComplete = nil} )
...
trans5 = transition.to(  badStar010 , { time =1000, x = math.random (70,display.contentWidth), y = math.random (70,display.contentHeight), onComplete = nil } )

trans6 = transition.to(  badStar011, { time =1000, x = math.random (70,display.contentWidth), y = math.random (70,display.contentHeight), onComplete = nil } )

trans7 = transition.to(  badStar012 , { time =1000, x = math.random (70,display.contentWidth), y = math.random (70,display.contentHeight), onComplete = nil } )
...
transition.to( badStar010, { time = 10, alpha = 0, onComplete = nil} );
...
transition.to( badStar011, { time = 10, alpha = 0, onComplete = nil} );
...
transition.to( badStar012, { time = 10, alpha = 0, onComplete = nil} );
...
audio.setVolume( 0.30, {channel=5} )
 ...
 function rotate ()
    if ( reverse == 0 ) then
      print "front"
        reverse = 1
        transition.to( badStar010, { rotation=-360, time=500, transition=easing.inOutCubic } )
        transition.to( badStar011, { rotation=-360, time=500,  delay = 3000, transition=easing.inOutCubic } )
        transition.to( badStar012, { rotation=-360, time=500,  delay = 6000, transition=easing.inOutCubic } )
    else
      print "back"
        reverse = 0
         transition.to(badStar010, { rotation=360, time=500, transition=easing.inOutCubic } )
...
if event.phase == "began"  and count == 10 then 
print "touch swirl"
cleanUp();
audio.play(soundBeep, {channel = 5})
audio.stop( Tick )
...
storyboard.gotoScene( "house01", zoomOutIn,400)
...
function  starcount( event )
  ...
  print( metadata.Starcount05 )
  metadata.Starcount05 = metadata.Starcount05 +1 
...
function timerDown()
   ...
   timeLeft.text = metadata.timeLimit
    metadata.timeLimit2=timeLeft.text
     if(metadata.timeLimit==5)then
     
     print "5 seconds left"

Tick = audio.play( soundClock,{channel = 1, onComplete = nil} )
...
coverIntro = transition.to(cover1,{time=10, x=display.contentCenterX*7, onComplete=showAds})
...
ID2=timer.performWithDelay( math.random(2000,20000), helpers02, 1)
...
ID3=timer.performWithDelay( math.random(2000,15000), helpers03, 1)


function Move01 ( )

TransMove01=transition.to(  enemy03 , { onStart = PlaySound , x = display.contentWidth+600 , time = 7000, delay = 2000, rotation = -360, onComplete = Move02 } )

end

function Move02 ( )

TransMove02=transition.to(  enemy03 , { onStart =  PlaySound , x = display.contentWidth-600, y=math.random( 100,500), time = 7000, delay = 2000, rotation = 360, onComplete = Move01 } )
 
end


MoveAcross=timer.performWithDelay( 4000, Move01, 1 ) 

function  PlaySound ()
  

Warn01=audio.play(soundEnmey02, {channel = 6})
--audio.fadeOut(soundEnmey, {time= 3000 } );
--audio.setMaxVolume( 1, {channel=5} )
...
badStar010:addEventListener( "touch", BadstarTouch01)
badStar011:addEventListener( "touch", BadstarTouch02)
badStar012:addEventListener( "touch", BadstarTouch03)
...
display.remove( badStar010 );
display.remove( badStar011 );
display.remove( badStar012 );
