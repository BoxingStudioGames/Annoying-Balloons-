...
-- background 
 local background = display.newImageRect("bg/level8.png", 320, 570)
...
function growSwirl ( )
    ...
    transition.to( swirl, { x = display.contentCenterX, y = display.contentCenterY, xScale = xFacter+1, yScale = yFacter+1 } )
 ...
 function rotate ()
    if ( reverse == 0 ) then
      ...
      print "back"
        ...
        transition.to( enemy04,{ rotation=360, time=500, xScale= 1, yScale = 1, alpha =  0.10, delay = 6000, transition=easing.inOutCubic } )
        transition.to( enemy05,{ rotation=360, time=500, xScale= 1.2, yScale = 1.2, alpha =  0.10, delay = 6000, transition=easing.inOutCubic } )
...
if event.phase == "began"  and count == 56 then 
print "touch swirl"
cleanUp();
audio.play(soundBeep, {channel = 5})
audio.stop( Tick )
...
storyboard.gotoScene( "backworld0102", zoomOutIn,400)
