...
-- background 
 local background = display.newImageRect("bg/levelup.png", 320, 570)
...
transH07 = transition.to( sub, {time = math.random( 5000,8000), delay = 2000, x = math.random(100,500), y = display.contentHeight-700, onComplete = nil} )
...
transH08 = transition.to( subp, {time = math.random( 5000,12000), delay = 2000, x = math.random(100,500), y = display.contentHeight-700, onComplete = nil} )
...
transH09 = transition.to( subps, {time = math.random( 2000,5000), delay = 2000, x = math.random(100,500), y = display.contentHeight-700, onComplete = nil} )
...
cover1 = display.newImage("bg/cover.png")
...
trans14 = transition.to(  badStar07 , { time=1000, x = math.random (70,display.contentWidth), y = math.random (70,display.contentHeight), onComplete = nil } )

trans15 = transition.to(  badStar08, { time=1000, x = math.random (70,display.contentWidth), y = math.random (70,display.contentHeight), onComplete = nil } )

trans16 = transition.to(  badStar09 , { time=1000, x = math.random (70,display.contentWidth), y = math.random (70,display.contentHeight), onComplete = nil } )
...
transition.to( badStar07, { time = 10, alpha = 0, onComplete = nil} );
gameOver();
...
transition.to( badStar08, { time = 10, alpha = 0, onComplete = nil} );
...
transition.to( badStar09, { time = 10, alpha = 0, onComplete = nil} );
...
coverIntro = transition.to(cover1,{time=10, x=display.contentCenterX, onComplete=resumeReady})
...
if (count == 0 and levels[8] == 2 ) then 

print "you have unlocked level 8"
...
function  SaveData07 ( event )
  ...
  print ("Every Is okay on level 6 ");
...
transH01 = transition.to( helper01, {time = 5000, delay = 2000, y = display.contentHeight-700, onComplete = nil} )


end
timer.performWithDelay( math.random(2000,5000), helpers, 1)
---------------------------------------------------------------

function Move01 ( )

TransMove01=transition.to(  enemy04 , { onStart = PlaySound , x = display.contentWidth+600 , time = 3000, delay = 1000,  alpha = 1, onComplete = Move02 } )

end

function Move02 ( )

TransMove02=transition.to(  enemy04 , { onStart =  PlaySound , x = display.contentWidth-600, time = 3000, delay = 1000, alpha = 0, onComplete = Move01 } )
 
end


MoveAcross=timer.performWithDelay( 2000, Move01, 1 )

function  PlaySound ()
  

AudioSound=audio.play(soundEnmey02, {channel = 6})
--audio.fadeOut(soundEnmey, {time= 3000 } );
--audio.setMaxVolume( 1, {channel=5} )
audio.setVolume( 0.70, {channel=6} )
 ...
 function rotate ()
    if ( reverse == 0 ) then
      print "front"
        reverse = 1
        transition.to( badStar07, { rotation=-360, time=500, transition=easing.inOutCubic } )
        transition.to( badStar08, { rotation=-360, time=500,  delay = 3000, transition=easing.inOutCubic } )
        transition.to( badStar09, { rotation=-360, time=500,  delay = 6000, transition=easing.inOutCubic } )
    else
      print "back"
        reverse = 0
         transition.to(badStar07, { rotation=360, time=500, transition=easing.inOutCubic } )
...
badStar07:addEventListener( "touch", BadstarTouch01)
badStar08:addEventListener( "touch", BadstarTouch02)
badStar09:addEventListener( "touch", BadstarTouch03)
...
display.remove( enemy03 );
display.remove( enemy04 );     
