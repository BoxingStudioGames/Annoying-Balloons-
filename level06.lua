...
badStar1 = balloonClass02.new02()
screenGroup:insert(badStar1)

badStar2 = balloonClass02.new02()
screenGroup:insert(badStar2)

badStar3 = balloonClass02.new02()
screenGroup:insert(badStar3)
...
trans5 = transition.to(  badStar1 , { time =1000, x = math.random (70,display.contentWidth), y = math.random (70,display.contentHeight), onComplete = nil } )

trans6 = transition.to(  badStar2, { time =1000, x = math.random (70,display.contentWidth), y = math.random (70,display.contentHeight), onComplete = nil } )

trans7 = transition.to(  badStar3 , { time =1000, x = math.random (70,display.contentWidth), y = math.random (70,display.contentHeight), onComplete = nil } )
...
transition.to( badStar1, { time = 10, alpha = 0, onComplete = nil} );
gameOver();
...
transition.to( badStar2, { time = 10, alpha = 0, onComplete = nil} );
...
transition.to( badStar3, { time = 10, alpha = 0, onComplete = nil} );
...
return true 
 ...
 function rotate ()
    if ( reverse == 0 ) then
      print "front"
        reverse = 1
        transition.to( badStar1, { rotation=-360, time=500, transition=easing.inOutCubic } )
        transition.to( badStar2, { rotation=-360, time=500,  delay = 3000, transition=easing.inOutCubic } )
        transition.to( badStar3, { rotation=-360, time=500,  delay = 6000, transition=easing.inOutCubic } )
    else
      print "back"
        reverse = 0
         transition.to(badStar1, { rotation=360, time=500, transition=easing.inOutCubic } )
...
storyboard.gotoScene( "space", zoomOutIn,400)
...
function  SaveData05 ( event )
  
  StarTableFive = {}

  print ("Every Is okay on level 5 ");

StarTableFive["LevelFiveStars"] = metadata.Starcount05

 saveTable (StarTableFive,"LevelFiveStars.txt")
...
hT01=timer.performWithDelay( math.random(2000,5000), helpers, 1)
...
hT02=timer.performWithDelay( math.random(2000,20000), helpers02, 1)
...
hT03=timer.performWithDelay( math.random(2000,15000), helpers03, 1)
...
badStar1:addEventListener( "touch", BadstarTouch01)
badStar2:addEventListener( "touch", BadstarTouch02)
badStar3:addEventListener( "touch", BadstarTouch03)
...
display.remove( badStar1 );
display.remove( badStar2 );
display.remove( badStar3 );
